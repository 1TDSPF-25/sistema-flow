...
function LoginFarmacia() {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormLoginData>({ shouldUnregister: true })

  const onSubmit: SubmitHandler<FormLoginData> = async (data) => {
    // Futuro - autenticacao (fetch, context, etc.)
    console.log('Dados de Login Validados', data)
    alert(`Tentativa de Login com Email: ${data.email}`)
  }
    ...
    <main className='min-h-screen bg-gray-50 flex items-center justify-center p-4'>
      ...
      <section className='bg-white shadow-xl rounded-lg p-8 w-full max-w-md'>
        ...
        <div className='mb-8 text-center'>
          ...
          <p className='text-gray-500 mt-1'>Entre com suas credenciais da Farmácia.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
          ...
          <div className='text-right'>
            <Link to='/recuperar-senha' className='text-sm text-blue-600 hover:text-blue-800 font-medium'>
              Esqueceu a senha?
            </Link>
          </div>
        ...
        <p className='text-center mt-6 text-gray-600'>
          Não tem uma conta?{' '}
          <Link to='/cadastro' className='text-blue-600 font-medium hover:text-blue-800'>
